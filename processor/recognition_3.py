...
class REC_Processor(Processor):
    ...
    def load_model(self):
        ...
        self.loss = nn.BCEWithLogitsLoss()  # 使用 BCEWithLogitsLoss
...
# def show_topk(self, k):
    ...
    def train(self):
        ...
        for i, (data, label) in enumerate(loader):
            ...
            # loss
            # print('loss: output, label', output.shape, label.shape)
        ...
        self.training_loss_log.append(self.epoch_info['mean loss'])  # 保存损失
    ...
    def test(self, evaluation=True):
        ...
        for data, label in loader:
            ...
            with torch.no_grad():
                output = self.model(data)
                probs = torch.sigmoid(output)  # 转为概率值，shape: (N, 1)
            # print('output', output.shape)
            # append results
            result_frag.append(probs.data.cpu().numpy())
            label_frag.append(label.data.cpu().numpy())
            # print('result_frag', result_frag.shape)
            # np.save('demo_video_gcn.npy', result_frag)
            # assert(0)
            # get loss
            # calculate loss
            loss = self.loss(output, label)
            loss_value.append(loss.item())  # 记录验证损失

            # calculate accuracy
            preds = (probs > 0.5).float()  # 预测类别：概率 > 0.5 为正类
            correct = (preds == label).float().sum()
            accuracy = correct / label.size(0)
            accuracy_value.append(accuracy.item())

        # 记录当前 epoch 的平均验证损失
        avg_validation_loss = np.mean(loss_value)
        self.validation_loss_log.append(avg_validation_loss)  # 保存损失
        ...
        if evaluation:
            ...
            mean_accuracy = np.mean(accuracy_value)
            self.epoch_info['Accuracy'] = mean_accuracy
            print('test Accuracy:', mean_accuracy)
