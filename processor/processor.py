...
class Processor(IO):
    ...
    def save_loss_logs(self):
        ...
        validation_acc_array = np.array(self.validation_acc_log)
        np.savetxt('training_loss_log.txt', training_loss_array)
        np.savetxt('validation_loss_log.txt', validation_loss_array)
        np.savetxt('validation_acc_log.txt', validation_acc_array)  
        print("Loss logs saved to 'training_loss_log.txt' and 'validation_loss_log.txt', 'validation_acc_log.txt'")
    ...
    def count_labels_in_dataset(self, dataset):
        c = Counter()
    ...
    def start(self):
        ...
        #===== 早停参数 =====
        patience = getattr(self.arg, 'patience', 20)  # 默认 20 个 epoch 不改善就停止
        min_delta = getattr(self.arg, 'min_delta', 1e-4)  # 最小改善阈值
        patience_counter = 0
        # ===== 早停参数结束 =====
       
        adj_save_dir = os.path.join(self.arg.work_dir, 'adjacency_data')  # ===== 新增：定义保存目录 =====
        # training phase
        if self.arg.phase == 'train':

            # ===== 训练前：保存 A_physical + A_eff(before_train) =====
            self.model.eval()
            # self.model.save_adjacency_data(save_dir=adj_save_dir, tag="before_train")
            self.model.train()
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    if val_loss < best_val_loss - min_delta:
                        ...
                        self.io.save_model(self.model, filename)
                        best_model_path = os.path.join(self.arg.work_dir, filename)
                        self.io.print_log(f'✅ New best model saved: {filename}')
                    else:
                        # 没有改善
                        patience_counter += 1
                        self.io.print_log(f'⏳ No improvement in val_loss. Patience: {patience_counter}/{patience}')
                    
                        if patience_counter >= patience:
                            self.io.print_log(f'🛑 Early stopping triggered at epoch {epoch+1}!')
                            break
                    # ===== 早停逻辑结束 =====
                    '''
                    # 使用acc 作为早停逻辑
                    if val_acc > best_val_acc + min_delta:
                        ...
                        patience_counter = 0
                        filename = f'epoch{epoch+1}_model_best_valacc_{best_val_acc:.6f}.pt'
                        self.io.save_model(self.model, filename)
                        best_model_path = os.path.join(self.arg.work_dir, filename)
                        self.io.print_log(f'✅ New best model saved by val_acc: {filename}')
                    else:
                        # 没有改善
                        patience_counter += 1
                        self.io.print_log(f'⏳ No improvement in val_acc. Patience: {patience_counter}/{patience}')
        ...
        if best_model_path is not None:
            ...
            self.model.eval()
            # self.model.save_adjacency_data(save_dir=adj_save_dir, tag="after_train")

            # test_loss, test_acc = self.test(split='test')
            test_loss, test_acc = self.test(split='test', save_attention=False) # 了绘制attention
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--min_delta', type=float, default=1e-4, help='minimum change to qualify as an improvement')
        # ===== 早停参数结束 =====
