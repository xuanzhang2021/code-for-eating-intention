...
class Processor(IO):
    ...
    def __init__(self, argv=None):
        ...
        # 初始化损失日志
        self.training_loss_log = []  # 用于存储训练损失
        self.validation_loss_log = []  # 用于存储验证损失
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                shuffle=True,
                num_workers=self.arg.num_worker * torchlight.torchlight.ngpu(self.arg.device),
                drop_last=True)
    ...
    def train(self):
        loss_value = []  # 用于记录当前 epoch 的所有损失
        for _ in range(100):
            self.iter_info['loss'] = 0
            self.show_iter_info()
            self.meta_info['iter'] += 1

            loss_value.append(self.iter_info['loss'])  # 将每次迭代的损失添加到列表中

        self.epoch_info['mean loss'] = 0
        self.training_loss_log.append(self.epoch_info['mean loss'])  # 将平均损失保存到日志
    ...
    def test(self):
        ...
        self.validation_loss_log.append(self.epoch_info['mean loss'])  # 将平均损失保存到日志
        self.show_epoch_info()

    def save_loss_logs(self):
        # 保存训练损失和验证损失到文件
        np.savetxt('training_loss_log.txt', self.training_loss_log)
        np.savetxt('validation_loss_log.txt', self.validation_loss_log)
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    if mae > tmp_mae:
                        mae = tmp_mae
                        filename = 'epoch{}_model_mae_{}.pt'.format(epoch + 1, mae)
                        self.io.save_model(self.model, filename)
                    self.io.print_log('Done.')

            # 保存损失日志
            self.save_loss_logs()
            # 绘制 Loss Convergence 曲线
            self.plot_loss_curve(self.training_loss_log, self.validation_loss_log)
