...
def type_1_partition(input, partition_size):  # partition_size = [N, L]
    B, C, T, V = input.shape
    # print(f"[type_1_partition] input.shape={input.shape}, partition_size={partition_size}")
...
def type_2_partition(input, partition_size):  # partition_size = [N, K]
    B, C, T, V = input.shape
    # print(f"[type_2_partition] input.shape={input.shape}, partition_size={partition_size}")
...
def type_3_partition(input, partition_size):  # partition_size = [M, L]
    B, C, T, V = input.shape
    # print(f"[type_3_partition] input.shape={input.shape}, partition_size={partition_size}")
...
def type_4_partition(input, partition_size):  # partition_size = [M, K]
    B, C, T, V = input.shape
    # print(f"[type_4_partition] input.shape={input.shape}, partition_size={partition_size}")
...
class SkateFormer(nn.Module):
    ...
    def forward(self, input):
        ...
        for layer in self.stem:
            output = layer(output)
        if self.index_t:
            time_index = torch.arange(T, device=output.device).unsqueeze(0).expand(B, T)  # [B, T]
            ...
            div_term = torch.exp(
                ...
                output.device)
            te[:, :, 0::2] = torch.sin(time_index.unsqueeze(-1).float() * div_term)
            te[:, :, 1::2] = torch.cos(time_index.unsqueeze(-1).float() * div_term)
